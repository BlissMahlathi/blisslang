...
impl Parser {
    ...
    fn parse_page(&mut self) -> ParseResult<PageNode> {
        ...
        let output    = OutputMode::from_str(attrs.get_str("output").unwrap_or("static"));
        let layout    = attrs.get_str("layout").map(str::to_string);
        let is_layout = attrs.get_bool("is_layout").unwrap_or(false);
        ...
        while !self.check(TokenKind::Dedent) && !self.at_end() {
            ...
            match &self.current().kind {
                ...
                TokenKind::Slot => {
                    self.advance();
                    let slot_attrs = self.parse_attr_list()?;
                    let slot_name = slot_attrs.get_str("name").unwrap_or("content").to_string();
                    self.skip_newlines();
                    sections.push(PageChild::Slot { name: slot_name });
                }
                TokenKind::Comment(c) => {
                    let c = c.clone();
                    self.advance();
        ...
        Ok(PageNode { name, attrs, route, output, layout, is_layout, sections })
    }
    ...
    fn expect_ident(&mut self) -> ParseResult<String> {
        match &self.current().kind.clone() {
            ...
            // Allow keywords to be used as identifiers in attribute-key position
            // (e.g. "from", "to", "type", "as" are valid attr keys but also keywords)
            other => {
                if let Some(s) = Self::keyword_to_ident_str(other) {
                    self.advance();
                    Ok(s)
                } else {
                    let tok = self.current();
    ...
    /// Map a keyword TokenKind back to its lowercase source-text identifier,
    /// for contexts where keywords double as attribute keys (from, to, type...).
    fn keyword_to_ident_str(kind: &TokenKind) -> Option<String> {
        let s = match kind {
            TokenKind::From  => "from",
            TokenKind::To    => "to",
            TokenKind::As    => "as",
            ...
            TokenKind::Or    => "or",
            TokenKind::Not   => "not",
            // PascalCase keywords (BuildSection, ForEach, etc.) are valid as
            // identifiers verbatim — used for things like UseDiv["Card"] names
            other => {
                let debug_str = format!("{}", other);
                if debug_str.chars().next().map(|c| c.is_uppercase()).unwrap_or(false) {
                    return Some(debug_str);
                }
                return None;
            }
        };
